 ...
 *  - `page` (СВОЙ профиль) — на полноценный экран `/records`. Плитки
 *    «Статистика» и «Любимые» отсюда убраны: у статистики теперь есть свой
 *    виджет на главной (там она с цифрами, а не просто вход), а любимые
 *    переехали отдельной строкой в меню профиля. Два входа в один раздел из
 *    соседних мест заставляют выбирать между ними.
...
export default function ProfileMetrics({
  ...
  // В своём профиле остаются ТОЛЬКО рекорды — остальное живёт своими входами
  // (см. описание сверху).
