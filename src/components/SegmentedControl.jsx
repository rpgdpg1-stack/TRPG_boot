...
/**
 * Сегмент-контрол — ОДИН на проект (DS-008). Пилюля-контейнер, внутри
 * равные сегменты, активный залит и подписан акцентом.
 *
 * Раньше назывался PeriodSwitcher и считался «переключателем периода», хотя
 * принимает произвольные items — из-за узкого имени такую же пилюлю рисовали
 * заново ещё в семи местах (место тренировки, длина бассейна, режим пикера,
 * дни конструктора). Каждая копия могла разъехаться самостоятельно: в
 * конструкторе так и вышло — контейнер рассыпался, сегменты стали голыми
 * кнопками. Новый переключатель собирать ЗДЕСЬ, а не копировать вид.
 *
 * Сейчас используется на экране статистики, в модалке статистики профиля
 * и в карточке друга.
 *
 ...
 */
export default function SegmentedControl({ items, value, onChange, compact = false, style }) {
  return (
