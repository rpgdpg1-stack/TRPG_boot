...
/**
 * Переключатель места тренировки (Зал/Дом/Улица) на карточке программы
 * (закреплённые на главной, списки каталога). В шапке дня его больше нет.
 *
 ...
 * ВНИЗ (стеклянное меню), выбор запоминается (prefs аккаунта) пока юзер
...
export default function PlaceSwitcher({ program, value: cValue, onChange, locked = false, tag = false }) {
  ...
  // Живёт на КАРТОЧКЕ программы, перед буквами дней. Пилюля остаётся на месте;
  // тап раскрывает список ВНИЗ — тем же стеклянным меню с хайрлайном, что
  // долгое нажатие на программу или друга: один вид у всех контекстных меню.
  // Меню — портал поверх экрана, поэтому карусель и её обрезка ему не мешают.
  return (
    <>
      <button
        ref={pillRef}
        onClick={toggle}
        // Нажатие на тег не взводит долгое нажатие карточки под ним: палец на
        // теге значит «выбрать место», а не «открыть меню программы».
        onPointerDown={multi ? stop : undefined}
