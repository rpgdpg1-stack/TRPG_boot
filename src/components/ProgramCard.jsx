...
export default function ProgramCard({
  ...
  const cardStyle = {
    ...
    alignItems: 'stretch',
    gap: 0,
    // Карточка с футером («последняя тренировка» на главной) держит одну высоту
    // и когда тренировка идёт: тогда футер уходит, а на его место встают таймер
    // и N/M. Без фиксации карточка подпрыгивала бы в момент старта, а вместе с
    // ней ехала бы вся страница.
    minHeight: lastTrained || footer ? HOME_CARD_MIN_H : styles.card.minHeight,
...
const styles = {
  ...
  ltLabel: { fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'var(--text-caption-size)', letterSpacing: '1.5px', color: 'var(--text-faint)' },
