 ...
 * `current_user_id()` внутри функции. Параметр `p_user_id` остался в сигнатуре
 * как исторический: фронт его шлёт, но функция его перезаписывает и ему НЕ верит.
 *
 * Так стало не всегда. До 02.09.2026 пятнадцать функций определяли человека
 * ТОЛЬКО по этому параметру и были открыты анонимной роли — посторонний мог
 * читать и менять чужие данные без входа (SEC-001). Правило «параметр
 * игнорируется» было записано здесь по остальным функциям и на деле не
 * выполнялось. Теперь выполняется — и обязано выполняться в каждой НОВОЙ
 * `api_`-функции: личность только из `current_user_id()`, права только
 * `authenticated`, никогда `PUBLIC` и `anon`.
