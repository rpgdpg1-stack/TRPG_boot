---
name: trpg-workflow
description: "Входная точка для ЛЮБОГО запроса по проекту TRPG (Telegram Mini App, фитнес-трекер на Vite+React+Supabase): правки кода, баги, обсуждение задач, добавление программ, ревью, коммиты. Отсюда маршрутизация в остальные скилы TRPG."
...
Проект **TRPG** — фитнес-трекер, Telegram Mini App. Ставка на простоту и минимализм:
игровые механики выпилены (см. «Отказ» ниже), название TRPG осталось историческим.
...
- **Стек:** Vite + React 18 + React Router 6 + Supabase + Telegram WebApp SDK.
  **Боевой хостинг — Timeweb**, домен `trpg1.ru`: именно туда смотрит Mini App
  в Telegram. Vercel — ЗАПАСНОЙ аэродром (`*.vercel.app`), не основной.
  Медиа на Selectel S3, бакет `trpg`. Стили — CSS (НЕ Tailwind).
...
Скилы живут в репозитории: `.claude/skills/{trpg-workflow,trpg-supabase,trpg-ui,trpg-ds-review,trpg-figma}/SKILL.md`.
...
**Как именно писать и разбивать скилы** — по скилу **skill-author** (глобальный, уровень
аккаунта): сначала ищем, куда дописать в существующий, новый заводим только если это
отдельная область со своим триггером. Скил длиннее ~500 строк разбиваем на ядро + `reference/`.
`trpg-ui` уже так устроен: ядро в `SKILL.md`, частности в
`trpg-ui/reference/{components,modals,screens,rules}.md` — новые правила класть в тот файл,
к которому обращаются по тому же поводу, а не в конец ядра.
...
## Куда всё уезжает и чем это видно

**Боевой контур:** `main` → Timeweb → `trpg1.ru`. Именно этот адрес открывает
Mini App в Telegram. Vercel (`*.vercel.app`) — запасной аэродром: он собирается
с того же репозитория, но людей туда не водят.

**Sentry различает их сам, по адресу** (`src/main.jsx`): хост оканчивается на
`vercel.app` → тег `environment: vercel`, иначе `production`. Настраивать
ничего не надо.

⚠️ Проверено 03.09.2026: ошибка с боевого `trpg1.ru` (iPhone, Telegram WebView)
дошла в Sentry с тегом `production` и полным стеком со ссылкой на файл. Были
сомнения, будет ли российский домен и хостинг работать со сбором ошибок в
европейском регионе Sentry, — работает. Связка Timeweb → Sentry EU рабочая,
проверять заново не нужно.

**Что деплоится само, а что руками:**

| Что | Как попадает в бой |
|---|---|
...
## Кросс-девайс настройка живёт в `user_prefs`, CloudStorage — только переезд

Источник правды для настроек, которые должны совпадать на телефоне и в
браузере, — таблица `user_prefs` в базе (`getPrefSync` / `setPref`).
Telegram CloudStorage виден ТОЛЬКО внутри Telegram: в браузере его нет,
код падает на localStorage, и настройки тихо расходятся между версиями.

CloudStorage остаётся законным в одном случае — разовый добор старого
значения. И тогда добор ОБЯЗАН заканчиваться записью в `user_prefs`:

```js
const fromAccount = getPrefSync(key, null)
if (fromAccount != null) { use(fromAccount); return }
cloudGet(key).then(v => {
  if (v == null) return
  use(v)
  setPref(key, v)   // ← без этой строки значение навсегда останется в Telegram
})
```

Ровно этой строки не хватало кругам заплыва: читались из CloudStorage при
каждом открытии и в браузер не доезжали никогда. Новый ключ, который должен
переехать, добавлять и в список `wanted` в `migrateFromCloud` (lib/prefs.js).

## «Ресурс не используется» — доказывать по ВСЕМ способам обращения

03.09.2026 я удалил четыре живые иконки (замеры тела, вход по почте, оба
сброса), решив, что они мёртвые. Проверка искала имя в кавычках — `'measure'`,
а в коде они передаются с префиксом: `icon: 'ui:measure'`. Регулярка такой
формат не поймала, отсутствие совпадений я принял за доказательство.

Иконки в проекте адресуются ДВУМЯ способами, и оба надо учитывать:

```
name="measure"        ← UiIcon напрямую
icon: 'ui:measure'    ← строки меню, CATEGORY_META
```

Правило шире иконок: прежде чем удалять ресурс как неиспользуемый, найти все
формы обращения к нему (префиксы, склейка строк, динамические имена) и
проверить каждую. Пустой результат одного `grep` — не доказательство, а
всего лишь один непроверенный способ. Дешевле оставить лишний файл, чем
выкинуть нужный: пропажу заметит человек, а не сборка.

## Ключ хранилища — только из `lib/storage-keys.js` (FE-006)

Строкой по месту не писать. Опечатка в ключе не ловится ни линтером, ни
сборкой, ни тестом: приложение не падает — просто перестаёт находить
сохранённое, а человек видит «прогресс пропал» (данные при этом целы, лежат
под старым ключом).

Там же лежат `USER_SCOPED_KEYS` и `USER_SCOPED_PREFIXES` — что снести при
входе другим аккаунтом. **Заводишь ключ с личными данными — добавь в список
сразу.** Забытый ключ даёт редкий и очень неприятный баг: чужие данные в своём
аккаунте, причём только у тех, кто входил двумя аккаунтами на одном устройстве.

## Данные, за которыми могут пойти дважды — через `cacheDedupe` (FE-001)

Кеш отвечает на «данные уже пришли?», но не на «за ними уже пошли?». Два
компонента, смонтированные в одном кадре, оба видят пустой кеш и оба уходят
в сеть — так история грузилась дважды на экране статистики.

```js
return cacheDedupe(key, async () => {
...
Кеширование остаётся ВНУТРИ загрузчика: только он знает про `canTrust`
(пустой ответ без ошибки не должен затирать настоящие данные).

## Пустой `catch` — только для необязательного (FE-003)

`.catch(() => {})` законен там, где сбой ничего не стоит: хаптика,
предзагрузка соседнего дня, аналитика. Где сбой означает потерю
пользовательских данных или молчаливую поломку сценария — `catchTo` из
`lib/report-error.js`:

```js
loadPrefs().catch(catchTo('app.loadPrefs'))
```

Поведение то же — приложение не падает, — но сбой виден в Sentry. Иначе
получается как было: настройки перестали переезжать из облака, а узнать
об этом неоткуда.
...
Второй заход (02.09.2026, вечер) дал ещё три таких: `FE-005` (все пять
«экранов без загрузки» оказались формами, синхронными списками или уже
имели `loading`), `PERF-002` (мёртвых иконок 4 из 40 на 1.4 КБ вместо
ожидавшихся 30–40 КБ — спрайт делать не стоило) и часть `DB-003`
(`idx_sets_exercise` предлагался к удалению, а у него 74 скана).

Третий заход (UX/UI) добавил ещё три: `UI-002` (CORS у бакета настроен
правильно — проверено `curl` с боевого домена и с localhost, все файлы
отдают `access-control-allow-origin`), `UI-003` (27px — это внутренности поля
веса, а его родитель 118px высотой; 48×24 — вообще скелетон загрузки, он не
кликабельный) и `UI-001`, отклонённый продуктово (см. trpg-ui: белый фон
превью нужен, чтобы тёмный тренажёр читался).

Итого: из примерно двадцати пяти проверенных находок **девять** не
подтвердились, оказались мельче заявленного или были отклонены по существу. Правило: перед правкой найти реальное место
и убедиться, что оно ведёт себя как описано. Отчёт — гипотеза, код — факт.

И наоборот, недооценка тоже бывает: `SEC-001` оказался опаснее, чем выглядел
(грант висел на `PUBLIC`, а не на `anon`), а `SB-001` — шире (дублировалась
вся функция проверки подписи, а не только порог свежести).
