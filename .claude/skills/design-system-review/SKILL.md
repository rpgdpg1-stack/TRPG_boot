...
### Минимальный набор до зрелой ДС (из аудита — порядок, БЕЗ лишнего)

Только легитимизация того, что УЖЕ используется (хардкодится), не новые сущности. Закрывать
постепенно, трогая соответствующие места:
1. **`--space-*`** — шкала отступов 4/8/12/16/20/24/32/40/48/64. Сейчас паддинги/гэпы хардкод
   (13/18/27…). Самый большой выигрыш в консистентности. Правило: брать из шкалы, не случайные.
2. **Типо-роли `--text-*`** — 6 уровней (display/title/body/label/caption/button) как size+weight+
   line-height. Сейчас размеры (13/15/17/18/22/28) живут в компонентах. Убрать «случайные 17px».
3. **`--z-*`** — base/nav/scrim/header/overlay/toast. Сейчас магические 90/95/100/9998/9999.
   Никаких `z-index:999999`.
4. **Статусы `--color-success/-warning/-error/-info`** — семантические ИМЕНА на существующие
   примитивы (зелёный/жёлтый/красный/голубой), 0 новых цветов. Сейчас `#E84545`/`#FF8C42`/`#FFD700`
   повторяются инлайном (Settings tone, модалки).
5. **`--shadow-dock` / `--shadow-modal`** — 2 токена на реально существующие тени (таб-бар/док, модалки).
6. **(Опц.) разбить `index.css`** на `styles/{tokens,base,utilities,keyframes}.css` (импорт в index.css) —
   навигация, когда файл разрастётся. Не обязательно.

Не переделывать разом: вводим токен → применяем в местах, которые и так трогаем.
