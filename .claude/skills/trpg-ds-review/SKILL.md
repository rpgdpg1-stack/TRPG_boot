---
name: trpg-ds-review
description: "Страж целостности дизайн-системы TRPG (код + Figma). Применять ПЕРЕД любой правкой интерфейса: сначала переиспользовать существующие токены и компоненты, новый заводить только с обоснованием. Отвечает за архитектуру и консистентность ДС, НЕ за UX-сценарии (это агент trpg-product-review)."
when_to_use: "Перед правкой интерфейса; «добавить новый цвет/токен/компонент», «а есть уже такое?», проверка консистентности, «не плоди дубли», сверка кода с Figma."
...
единому виду. Это про **целостность и токены**, не про UX-сценарии (ими занят агент `trpg-product-review`).
...
History Row, `StreakFlame` (Weekly Progress), `MuscleIcon`, `PlacePickerModal` (выбор места из меню карточки),
