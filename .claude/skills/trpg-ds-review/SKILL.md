---
name: trpg-ds-review
description: "Страж целостности дизайн-системы TRPG (код + Figma). Применять ПЕРЕД любой правкой интерфейса: сначала переиспользовать существующие токены и компоненты, новый заводить только с обоснованием. Отвечает за архитектуру и консистентность ДС, НЕ за UX-сценарии (это агент product-review)."
