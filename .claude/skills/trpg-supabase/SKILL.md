...
### ГРАБЛЯ: «проверил и вставил» — это не защита

`SELECT ... IF NOT FOUND THEN INSERT` в функции защищает только от
последовательных вызовов. Два одновременных оба проходят проверку и оба
вставляют. У нас так обходился лимит «одна тренировка в сутки»: оффлайн-очередь
досылает завершение ровно тогда, когда человек жмёт «Завершить» с другого
устройства — в истории появлялась лишняя тренировка, а серия росла дважды.

Лечится блокировкой строки владельца в начале функции:

```sql
PERFORM 1 FROM users WHERE id = p_user_id FOR UPDATE;
```

Она выстраивает вызовы ОДНОГО человека в очередь и не влияет на других.
Уникальный индекс надёжнее, но годится не всегда: если условие считается
выражением с подзапросом (у нас лимит по КАТЕГОРИИ программы через
`COALESCE(pr.category, w.program_id)`), в индекс такое не положить.

### ГРАБЛЯ: время и числа от клиента — ограничивать окном

`p_finished_at` приходит от клиента (так надо оффлайн-очереди, она досылает
настоящее время после возврата связи). Но лимит суток считался ОТ ЭТОГО
значения — и тридцать вызовов с разными датами давали месяц «тренировок»
мимо лимита. Правило: параметр, от которого зависит проверка, обязан иметь
границы.

```sql
IF p_finished_at > now() + interval '5 minutes'      -- часы устройства врут
   OR p_finished_at < now() - interval '7 days' THEN -- запас на любой оффлайн
  RAISE EXCEPTION 'finished_at out of range' USING ERRCODE = '22007';
END IF;
```

То же про числа: диапазон лучше ставить `CHECK` на таблице, а не проверкой
в функции — ограничение действует независимо от того, кто пишет.

### Сброс аккаунта — по списку таблиц, а не по памяти

`api_reset_my_progress` приводит аккаунт к состоянию только что
зарегистрировавшегося. Долго она чистила лишь `workouts` и серию, а остальное
«полное обнуление» переживало — просто потому, что список писали по памяти
и он отстал от схемы.

Правило: **заводишь таблицу с `user_id` / `owner_id` — сразу добавь её в
сброс.** Проверить, что ничего не забыто, можно запросом:

```sql
SELECT c.relname, string_agg(a.attname, ', ') AS чей
FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
JOIN pg_attribute a ON a.attrelid = c.oid AND a.attnum > 0
WHERE n.nspname = 'public' AND c.relkind = 'r'
  AND a.attname IN ('user_id','owner_id','author_id')
GROUP BY c.relname ORDER BY c.relname;
```

Каждая строка ответа должна либо встречаться в теле функции, либо иметь
записанную причину, почему её не трогаем. Сейчас таких причин три:
дружбы (двусторонние — вычеркнув себя, меняешь список другому человеку),
способ входа с `referral_code` (иначе человек теряет аккаунт) и `email_codes`
(служебные, истекают сами).

**Порядок удаления диктуют внешние ключи.** `program_days`,
`workout_exercises` и `user_favorite_exercises` ссылаются на `exercises` БЕЗ
каскада — значит упражнения удаляются последними, а до них надо снять
ссылки. Своё упражнение, отданное другу через «поделиться», удалить нельзя
вовсе: его архивируем (`archived_at`), иначе развалится программа друга.

### Снимаешь права — проверь, кто ими пользуется СНАРУЖИ

Права нужны не только приложению. По базе ходят GitHub Actions, и ходят они
не сервисным ключом, а тем, что положили в секрет. Я снял у `anon` право
UPDATE на `heartbeat` (SEC-004), написав в комментарии «пишет keepalive под
service_role» — и не проверил. Пинг делает PATCH публичным ключом, задача
упала с 401, база две недели не получала сигнала «я живая».

Перед `REVOKE` пройти по `.github/workflows/*.yml` и посмотреть, какие
запросы там есть и под каким секретом. Публичным ключом сейчас ходит
keepalive; бэкап и напоминания — сервисным.

После правки прав — прогнать задачи вручную (`gh workflow run <файл>`),
у всех четырёх есть ручной запуск. У напоминаний обязательно с
`-f dry_run=true`, иначе люди получат сообщения.

### Лимит из интерфейса ДУБЛИРУЕМ ограничением в базе

`maxLength` у поля и обрезка при сохранении живут в браузере — они не мешают
отправить запрос к базе напрямую с чем угодно: публичный ключ лежит в бандле.
Поэтому у каждого пользовательского лимита должна быть пара — `CHECK` на
таблице. Он действует независимо от того, кто пишет.

Ставить границу РОВНО такую же, как в интерфейсе, а не «с запасом»: обычный
человек её не заметит никогда (поле и так не даст ввести больше), а запас
только ослабляет защиту. Сейчас так закрыты:

| Что | Граница | Почему такая |
|---|---|---|
| Заметка | 280 символов | `NOTE_MAX_LENGTH` во фронте |
| Рабочий вес | 0–500 кг | мировой рекорд в приседе ~350 |
| Дистанция заплыва | 0–100 000 м | марафонский заплыв ~50 км |

Меняешь лимит во фронте — меняй и `CHECK`. У ограничения заметки для этого
есть `COMMENT` с напоминанием.

Опасность тут не «обман системы» — соревноваться в продукте не с кем.
Опасность в другом: одна абсурдная точка (9999 кг) расплющит весь график
весов в линию у нижнего края, и он станет нечитаемым, пока её не удалить
руками. А заметка на мегабайт — это квота бесплатного тарифа, общая для всех.

### ГРАБЛЯ: RENAME таблицы не трогает тела функций

`ALTER TABLE ... RENAME` обновляет политики RLS и индексы, но текст внутри
функций остаётся старым — и они ломаются при первом вызове. Перед
переименованием найти всех, кто ссылается:

```sql
SELECT proname FROM pg_proc
WHERE pronamespace = 'public'::regnamespace AND prosrc ILIKE '%старое_имя%';
```

Имена ограничений и последовательностей Postgres тоже НЕ переименовывает —
это нормально, они остаются историческими.

### ГРАБЛЯ: CREATE OR REPLACE не может убрать умолчания параметров

Если у функции есть `DEFAULT` у параметров, а в новой версии их не написать —
Postgres откажет: «cannot remove parameter defaults from existing function».
Смотреть текущую сигнатуру С умолчаниями:

```sql
SELECT pg_get_function_arguments(oid) FROM pg_proc WHERE proname = '…';
```

(`pg_get_function_identity_arguments` умолчания НЕ показывает — по нему
сигнатуру не восстановить.)
...
Отсюда правило на фронте (`src/lib/session.js`): перед чтением своих данных —
`canReadServer()`, перед записью ответа в кеш — `canTrust(error)`. Нет сессии →
не ходим и не кешируем, показываем сохранённое. Пренебрежение этим стоило
пользователю заметок и рабочих весов: пустота уехала в localStorage поверх
настоящих данных и держалась там неделю (август 2026).

## Напоминание о забытой тренировке (бот, 16.09.2026)

- `active_sessions.last_tick_at` — время последней ГАЛОЧКИ. Ставит `api_set_active_session`
  только когда набор `done` поменялся (сравнение как множеств `@>`/`<@`) или началась другая
  сессия. `updated_at` для порога НЕ годится: приложение пересохраняет сессию и при открытии дня.
- `active_sessions.reminded_at` — бот уже написал про эту сессию; сбрасывается той же функцией
  при новой галочке или новой сессии.
- `srv_stale_session_candidates()` — кому напомнить: активные, с `telegram_id`, `notify_nudge`,
  не старше 30 дней; галочки есть → 90 мин после последней, нет → 3 ч после старта. **Пороги
  дублируют `src/utils/workout-stale.js` — менять разом.**
- `srv_mark_session_reminded(user, started_at)` — отметка после отправки (и при 403/400: писать
  заблокировавшему повторно бессмысленно).
- Edge Function `stale-workout-remind` (`verify_jwt=false`, повтор безвреден). Кнопка —
  `startapp=open-stale` → приложение → модалка «Тренировка не завершена».
- Расписание: `pg_cron` каждые 15 мин → `net.http_post` (расширение `pg_net`) на функцию.
  **Первый запуск пишет живым людям** — перед включением прогнать `dry_run` и показать
  Дмитрию, кому уйдёт.

### Кнопки «Засчитать / Не засчитывать» в самом сообщении (16.09.2026)

- Edge Function `telegram-bot-webhook` принимает `callback_query`. Защита — секрет в заголовке
  `X-Telegram-Bot-Api-Secret-Token`: функция выпускает его САМА (`POST {"action":"setup"}`),
  хранит в `bot_config` и ставит вебхук. Секрета нет ни в репозитории, ни в переменных окружения.
  Не угадал секрет — отвечаем `ok` и молчим (ошибка заставила бы Telegram слать повторы).
- `callback_data` = `sw:c:<epoch>` / `sw:d:<epoch>`, где epoch — старт тренировки. Нажатие на
  СТАРОМ сообщении не тронет новую тренировку: сервер сверяет время (`srv_session_for_callback`,
  допуск 1.5 с).
- Завершение — `srv_finish_workout(p_user_id, ...)`: тело переехало из `api_finish_workout`,
  которая стала тонкой обёрткой (личность из `current_user_id()`). Копий логики лимита суток и
  серии быть не должно.
- **ГРАБЛЯ (поймана логами при проверке):** в `active_sessions.program_id` лежит СЛАГ (`split`),
  а `workouts.program_id` — id справочника (`prog_001`). Перевод делает клиент
  (`features/programs/registry.js`), у сервера его не было → `workouts_program_id_fkey`.
  Теперь переводит `srv_session_for_callback` (встроенные — CASE, своя программа — по владельцу,
  её строка `usr_<owner_id>`). Любая новая серверная запись тренировки обязана делать тот же перевод.
- **Состав отмеченного** приложение шлёт в сессию (`done_exercise_ids`): у встроенных программ
  состав дня лежит в коде клиента, сервер сам его не соберёт.
- Проверять без Telegram можно имитацией: взять секрет из `bot_config`, послать POST с телом
  `callback_query` и этим заголовком. Тестовому аккаунту (22) на время проверки выдаётся
  служебный `telegram_id` — после проверки вернуть `null`.
...
## Edge Functions деплоятся ВРУЧНУЮ

Автодеплоя нет: правка в репозитории не означает правку в бою. Функций четыре.
Три про вход (`telegram-auth`, `email-request-code`, `email-verify-code`) —
сломается деплой, никто не войдёт, включая тебя. Четвёртая —
`stale-workout-remind` (напоминание о забытой тренировке) — вход не трогает,
её можно деплоить без похода в Telegram: проверка — вызов с `{"dry_run": true}`.

Правило: деплоить только тогда, когда сразу после этого можно открыть Mini App
в Telegram и убедиться, что вход прошёл. Если проверить негде — правка ждёт
...
## Медиа под пол — база отдаёт ОБА варианта, выбирает клиент

У упражнения две пары ссылок (`preview_url_male/female`, `video_url_male/female`) плюс старые
`preview_url`/`video_url` как запасной путь. Правило выбора («свой вариант → чужой → старая
ссылка») живёт в ОДНОМ месте — `src/lib/gender-media.js`. Поэтому **любая RPC, отдающая медиа
упражнения, обязана отдавать оба варианта**, а не выбирать сама: иначе правило раздвоится.

Так сделаны `api_get_favorite_exercises`, `srv_user_records` (поле `strength`) и
`api_get_user_public_profile` (любимые + рекорды). Публичный профиль отдаёт ещё и **`sex`
владельца** — картинки в чужом профиле показываются по полу ХОЗЯИНА, а не смотрящего: женщина
показывает друзьям женские приседания. Пол приходит только ради выбора картинки, в интерфейсе
он нигде не пишется.

На клиенте кеши (любимые, рекорды, профиль друга) хранят СЫРОЙ ответ, а пол применяется на
выходе — смена пола в настройках меняет картинки сразу, без ожидания протухания кеша.
