...
- **Удаление — архивация** (`archived_at`), не `delete`. На упражнение ссылаются
  прошлые тренировки и история весов; стереть строку значит переписать прошлое.
  Функция вынимает его из программ владельца и пересобирает `order_num` подряд —
  иначе дыры в нумерации разъедутся со свапами.
