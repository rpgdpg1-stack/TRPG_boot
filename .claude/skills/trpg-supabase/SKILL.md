...
**После каждой правки базы — прогнать линтер:** `get_advisors(type: 'security')`
и `get_advisors(type: 'performance')`. Он ловит то, что глазами не видно: функцию
без `SET search_path`, таблицу с RLS без политик, внешний ключ без индекса. Разбор
...
- `auth_leaked_password_protection` — паролей у нас нет вообще (вход по коду
  и по Telegram), настройка не про нас;
- `unused_index` — на наших объёмах планировщику проще прочитать таблицу целиком,
  «не использован» ≠ «не нужен». Не удалять: понадобятся, когда данных станет больше.
...
- Недельная серия считает ТРЕНИРОВКИ, а не дни: `weekly_streak + 1` на каждую засчитанную,
  сброс с понедельника по Москве. Потолка 7 больше НЕТ — с лимитом на раздел за день
  засчитывается до 4 (по числу категорий), за неделю теоретически до 28. `LEAST()` не нужен.
...
- **Лимит — НА РАЗДЕЛ**: 1 засчитанная тренировка в сутки (Москва) в каждой категории
  (`programs.category`), второй раз в ТОМ ЖЕ разделе → `already_completed_today=true` без
  нового ряда/баллов. Силовая + заплыв в один день засчитываются обе. Программы нет в
  справочнике — ключом лимита служит сам `program_id` (лимит не должен исчезать вовсе).
  Миграция `day_limit_per_category.sql`. Осталось из старого TODO: жёсткая блокировка
  кнопки «Завершить». См. [[proj-trpg-history-calendar]].
