 ...
 */
// Заголовки сообщений (итоги, паузы) идут БЕЗ эмодзи: у каждого уведомления
// есть картинка, она и объясняет, о чём оно. Значок перед заголовком повторял
// её и делал ленту пёстрой.
const E = {
  ...
  // Люди-разделы — тем же светлым тоном кожи, что бицепс: иначе рядом
  // с ним они выходят жёлтыми и сообщение пестрит двумя палитрами.
  strength: '🏋🏻',   // силовая
  pool: '🏊🏻',       // плавание
  cardio: '🏃🏻',
  stretch: '🧘🏻',
...
export function weeklyDigest({ totalCount, totalMinutes, breakdown }) {
  const lines = ['<b>Итоги недели</b>', '', headLine(E.workouts, totalCount, totalMinutes), '']
...
/**
 * Строка программы: «🏋🏻 Сплит 2 (день A)» или «🏊🏻 Заплыв 45 (750 м)».
 *
