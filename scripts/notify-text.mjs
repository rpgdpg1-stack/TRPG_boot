...
function pauseTitle(days) {
  ...
  return 'Прошлая неделя без тренировок'
}
...
export function nudge({ daysSince, programName, category, lastDay, estMinutes }) {
  ...
  if (daysSince >= 28) {
    return [
      `<b>${title}</b>`, '',
      'Программы тренировок никуда не делись — можно',
      'вернуться с любого дня, не начиная сначала.'
    ].join('\n')
  }

  const lines = [`<b>${title}</b>`, '']

  // Две-три недели: снимаем планку входа до одного упражнения.
  if (daysSince >= 14) {
    lines.push('Не обязательно возвращаться в полный день.')
    lines.push('Одного упражнения достаточно, чтобы снова начать.')
    lines.push('')
  }
  ...
  if (name) {
    ...
    lines.push(isPool || !lastDay
      ? `В программе: ${name}`
      : `В программе: ${name}, день ${lastDay}`)
    // Оценку показываем только у недельного пинка: тому, кто выпал на три
    // недели, «~50 минут» читается как приговор, а не как подсказка.
    ...
    if (daysSince < 14 && estMinutes && showEstimate(name)) {
...
export function nudgeButtonLabel(daysSince) {
  ...
  if (daysSince >= 14) return 'Открыть'
  return 'Начать'
}
